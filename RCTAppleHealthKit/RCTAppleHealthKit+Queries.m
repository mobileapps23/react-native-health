...
- (void)makeIntervalsForStatisticsCollection:(HKQuantityType *)quantityType
								  ...
								  completion:(void (^)(NSArray *, NSError *))completionHandler
{
	NSPredicate *predicate = [RCTAppleHealthKit predicateForStatisticsBetweenDates:startDate endDate:endDate];

	// Create the query
	HKStatisticsCollectionQuery *query = [[HKStatisticsCollectionQuery alloc] initWithQuantityType:quantityType
																		   quantitySamplePredicate:predicate
																						   options:HKStatisticsOptionDiscreteMin
	...
	query.initialResultsHandler = ^(HKStatisticsCollectionQuery *query, HKStatisticsCollection *results, NSError *error) {
		...
		NSDate* fromDateForEnumerate = startDate;
		if (fromDateForEnumerate == nil) {
			fromDateForEnumerate = [NSDate dateWithTimeIntervalSince1970: 0];
		}
		[results enumerateStatisticsFromDate:fromDateForEnumerate
									  toDate:endDate
								   withBlock:^(HKStatistics *result, BOOL *stop) {
									   if (result.minimumQuantity != nil) {
										   NSDateInterval* interval = [[NSDateInterval alloc] initWithStartDate:result.startDate endDate:result.endDate];
										   [intervals addObject: interval];
									   }
								   }];
		completionHandler(intervals, error);
	};
...
- (void)fetchStatisticsCollection:(RCTStatisticRequest *)request
	...
	query.initialResultsHandler = ^(HKStatisticsCollectionQuery *query, HKStatisticsCollection *results, NSError *error) {
		...
		NSDate* fromDateForEnumerate = request.startDate;
		if (fromDateForEnumerate == nil) {
			fromDateForEnumerate = [NSDate dateWithTimeIntervalSince1970: 0];
		}
		[results enumerateStatisticsFromDate:fromDateForEnumerate
								   ...
								   withBlock:^(HKStatistics *result, BOOL *stop) {
									   HKQuantity *quantity = [request fetchValue:result];
									   if (quantity) {
										   double value = [quantity doubleValueForUnit:request.unit];
										   NSLog(@"%@-%@: %f", result.startDate, result.endDate, value);
		...
		if (completion) {
			...
			dispatch_async(dispatch_get_main_queue(), ^{
				...
				for (HKQuantitySample *sample in results) {
					...
					NSString *startDateString = [RCTAppleHealthKit buildStringFromDateForStatistics:interval.startDate];
					NSString *endDateString = [RCTAppleHealthKit buildStringFromDateForStatistics:interval.endDate];
